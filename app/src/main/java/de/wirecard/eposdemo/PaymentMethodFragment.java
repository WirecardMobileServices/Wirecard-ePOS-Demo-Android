...
import de.wirecard.eposdemo.adapter.paymentMethod.OnPaymentMethodClickListener;
import de.wirecard.eposdemo.adapter.paymentMethod.PaymentMethodAdapter;
import de.wirecard.rxparallel.SingleParallel;
...
public class PaymentMethodFragment extends AbsFragment<View> implements OnPaymentMethodClickListener {
    ...
    private Relay<Event> eventRelay = BehaviorRelay.create();
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        if (arguments != null) {
            String amountString = arguments.getString(MainActivity.AMOUNT);
            amountValue = new BigDecimal(amountString);
        }

        initViews();
        initParallelEventsHandler();
    ...
    private void initParallelEventsHandler() {
    ...
    @Override
    public void onPaymentMethodClicked(AppPayment appPayment) {
        doPayment(appPayment, eventRelay);
    }

    private void doPayment(AppPayment appPayment, Relay<Event> eventRelay) {
        showLoading();
        ...
        APurchasePayment payment = createPayment(appPayment);
        Request request = createRequest(payment, saleAmount, cashRegisterId, saleItem);

        SaleBuilder.newBuilder()                       // builder will guide you what you have to put
...
//                .addAlipayPayment(new BigDecimal("10"), "scannedConsumerIdFromAlipayApplication")
        ...
        if (request == null) {
            Toast.makeText(getContext(), R.string.unsupported_payment_combination, Toast.LENGTH_LONG).show();
            loadingFinished();
            return;
        }

        SingleParallel<String, Event> purchaseOperation = null;
        if (request instanceof PurchaseRequest)
            purchaseOperation = eposSdk.sales().operation().purchase((PurchaseRequest) request);
    ...
    private APurchasePayment createPayment(AppPayment appPayment) {
        APurchasePayment payment = null;
        if (appPayment == AppPayment.CARD)
            payment = new CardPurchasePayment(amountValue);
        else if (appPayment == AppPayment.CASH)
    ...
    private Request createRequest(APurchasePayment payment, BigDecimal saleAmount, String cashRegisterId, List<SaleItem> saleItem) {
        ...
        else if (payment instanceof TerminalAuthorizationPayment) {
            request = new TerminalAuthorizationRequest(
                    saleAmount,
                    CURRENCY.getCurrencyCode(),
                    "demo terminal authorization note",
                    null,
                    null,
                    null,
                    null,
